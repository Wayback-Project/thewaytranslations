# 1-John — NT Change Log

## 2026-09-06 06:32 UTC — Adversary/Slanderer source distinction
- Scope: 1-John 3:8, 10
- Change: Satan/ha-Satan/Satanas-derived title language is rendered “the Adversary”; diabolos-derived language is rendered “the Slanderer” (generic “a slanderer” where context requires).
- Rationale: Preserve distinct Hebrew/Semitic and Greek lexical force instead of flattening both terms into the conventional “devil.”
- Source track: Aramaic-priority with Greek check
- Impacted files: original-documents/rest_of_new_testament_restorative_translation.txt, current-form-documents/the-way-current.epub
- Reviewer status: accepted and implemented in September 2026 release

## 2026-04-08 12:01 UTC — Pass 1 baseline classification
- Scope: Book-level source-priority + audit setup
- Change: Added this book to NT logging framework and assigned track `Aramaic-priority with Greek check`.
- Rationale: Enforce explicit discernment and reproducible review path.
- Source track: Aramaic-priority with Greek check
- Impacted files: original-documents/rest_of_new_testament_restorative_translation.txt, change-logs/new-testament/INDEX.md
- Reviewer status: complete (pass 1)
